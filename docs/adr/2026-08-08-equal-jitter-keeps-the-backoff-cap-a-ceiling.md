...
the number an operator can rely on, and the worst case for a retried job is bounded at ten minutes
without qualification. Two workers failing at the same moment come back at least a few seconds
apart on the very first retry, and further apart as the delay grows.

Because the source is injectable, the schedule is pinned by a table-driven test at both ends of the
jitter range rather than by sampling.
