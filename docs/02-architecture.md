...
| [**ACU totals are summed from the window's remediations**](./adr/2026-08-08-acu-totals-are-summed-from-remediations-and-labelled-by-ledger-coverage.md) | `acu_ledger` is an organisation-wide daily total and cannot be cut at a window boundary, so using it as the numerator would inflate the cost of every fix by unrelated Devin usage. `cost.acus_total` sums `acus_consumed` over the window; `cost.source` reports whether the consumption sync covers every day of it, which is provenance rather than a second arithmetic path ([05](./05-devin-integration.md#degradation)). |
...
| [**A figure with an empty denominator is blank, not zero**](./adr/2026-08-08-blank-a-figure-whose-denominator-is-empty.md) | Every rate and percentile in [07](./07-observability.md) divides by a funnel stage, and the response schema types them all as plain numbers — so a window that merged nothing arrives as `"autonomy": 0`, `"usd_per_fix": 0` and `"to_merge": {"p50": 0}`, indistinguishable from instant, free, fully autonomous merges. Panels gate each figure on the funnel count it was computed over and render `—` with the reason instead. A zero that is a measurement, such as ACUs spent, is kept. |
| [**The cost panel labels its ACUs and its dollars separately**](./adr/2026-08-08-cost-panel-labels-acu-and-dollar-provenance-separately.md) | `cost.source` says only where the ACU counts came from. Devin never reports dollars: cost per fix is always Sentinel's own conversion at `ACU_UNIT_COST_USD`, a locally configured contract price ([09](./09-operations.md)). A single badge driven by `source` would attribute the panel's most consequential number to Devin, so the panel states both provenances, always ([05](./05-devin-integration.md)). |
| [**A figure with no denominator is an em dash**](./adr/2026-08-08-an-undefined-figure-is-an-em-dash.md) | Panels render the figure the analytics API computed and read the funnel only to decide whether it exists: nothing labelled makes the success rate undefined rather than 0%, because "0% success" claims the pipeline failed when nobody asked it to do anything. Recomputing a rate inside a panel would also print a different number from the one its neighbour shows, since the payload's ACU figure is already display-rounded ([07](./07-observability.md)). |
| [**The analytics API serves the metrics module's own schema**](./adr/2026-08-08-the-metrics-module-is-the-published-schema.md) | The summary body is fixed in [07](./07-observability.md) and already transcribed twice — as `TypedDict`s in `sentinel.analytics.metrics` and as interfaces in `dashboard/src/api.ts`, which nine panels index into. `metrics.SummaryJson` is handed to FastAPI as the response model rather than restated as Pydantic models in the router, so a dropped key fails the request instead of reaching the dashboard, and `parse_window` stays the only definition of a valid window — the boundary translates its `ValueError` into a `400` naming the 365-day ceiling. |
