...
| [**A working session waiting for a user is blocked**](./adr/2026-08-08-a-stalled-session-is-blocked.md) | `status_detail: waiting_for_user` on a session Devin still considers at work is a question asked of nobody: the only messages Sentinel sends are the resume messages of the fix loop, and neither can arrive mid-session. It escalates on the first observation with `blocked_reason = session_waiting_for_user`, because no number of further polls can produce an answer. A `suspended` or `exit` session with the same detail is idling between laps — the normal condition of every remediation in CI or review — and is left alone ([05](./05-devin-integration.md)). |
